...
app.post('/api/study-log/:id/like', async (req, res) => {
  ...
  if (ex) { await supabase.from('study_log_likes').delete().eq('id', ex.id); res.json({ success:true, liked:false }); }
  else { await supabase.from('study_log_likes').insert({ study_log_id, user_id }); res.json({ success:true, liked:true }); }
});
